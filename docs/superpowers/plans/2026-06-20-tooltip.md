...
```typescript
import { Tooltip, VClosePopper, Dropdown as VDropdown, Menu as VMenu } from "floating-vue";
```
...
```html
v-tooltip="{ content: tooltip, disabled: (tooltip?.length ? false : true) || hideTooltip, placement:
'auto', distance: 12, }"
```
...
```html
v-tooltip="{ content: tooltip, disabled: (tooltip?.length ? false : true) || hideTooltip, offset:
12, }"
```
...
```html
v-tooltip="{ content: `${word}, ist das heutige Wort des Tages`, distance: 10, placement:
tooltipPlacement, }"
```
...
```html
v-tooltip="{ content: `${word}, ist das heutige Wort des Tages`, offset: 10, placement:
tooltipPlacement, }"
```
...
```html
v-tooltip="{ content: 'Klick auf das Wort um mehr zu erfahren!', distance: 10, shown: showTooltip,
placement: 'bottom', }"
```
...
```html
v-tooltip="{ content: 'Klick auf das Wort um mehr zu erfahren!', offset: 10, shown: showTooltip,
placement: 'bottom', }"
```
