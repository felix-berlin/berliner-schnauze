# dropdown-popover Specification

## Purpose

Define the current-state and fixed-state behavior of the shared `DropdownPopover.vue`
component: deterministic panel identity unique across the whole page (not just one
Vue app/island), native Popover-API-driven open/close before and after hydration, and
SSR-safety for any consumer that chooses to drop `client:only`.
