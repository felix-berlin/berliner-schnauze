...
"""
import csv, glob, html, json, random, re, sys
...
def toks(s):
    s = html.unescape(re.sub(r"<[^>]+>", " ", s or "")).lower().translate(TR)  # infoText is WYSIWYG HTML
...
def cross_fields(w, n8):
    """Fields whose joined text (or a field boundary in any order) forms a shared 8-gram.

    Items of a field are joined and adjacent fields (as rendered on the page) can
    complete a window that no single item reaches, so every ordered pair is checked.
    Title/article are facts and never cleared, but count as context.
    """
    it = field_items(w.get("wordProperties"))
    seq = {f: toks(" ".join(x for x in it[f] if x)) for f in FIELDS}
    seq["title"] = toks(w["title"] or "")
    hit = set()
    for f in FIELDS:
        if any(g in n8 for g in grams(seq[f], 8)):
            hit.add(f)
    for a in seq:
        for b in seq:
            if a == b or not seq[a] or not seq[b]:
                continue
            t = seq[a] + seq[b]
            for i in range(max(0, len(seq[a]) - 7), min(len(seq[a]), len(t) - 7)):
                if " ".join(t[i:i + 8]) in n8:
                    hit.update(f for f in (a, b) if f in FIELDS)
                    break
    return hit
...
def main():
    ...
    for w in wp:
        ...
        xs = cross_fields(w, n8)
        clear = {f: trigger(sc[f]) or f in xs for f in FIELDS}
        rows.append((w["databaseId"], title, w["status"], sset, "A" if any(clear.values()) else ("B" if sset == "S1" else "C"), sc, clear))
    with open(sys.argv[1] if len(sys.argv) > 1 else "reports/schlobinski-scan.csv", "w", newline="", encoding="utf-8") as f:
        ...
        for r in sorted(rows, key=lambda r: r[0]):
            cw.writerow(list(r[:5]) + [v for x in FIELDS for v in (*r[5][x], int(r[6][x]))])
    cnt = Counter((r[3], r[4]) for r in rows)
    for k in sorted(cnt):
        print(k, cnt[k])
    print("fields to clear", {f: sum(r[6][f] for r in rows) for f in FIELDS})
