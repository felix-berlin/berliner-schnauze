...
describe("PwaCacheActions", () => {
  it("renders 3 buttons", () => {
    const wrapper = mount(PwaCacheActions, { props: { hasNoBuckets: false, isLoading: false } });
    expect(wrapper.findAll("button")).toHaveLength(3);
  });

  it("Aktualisieren disabled when isLoading", () => {
    const wrapper = mount(PwaCacheActions, { props: { hasNoBuckets: false, isLoading: true } });
  ...
  it("Alles leeren disabled when hasNoBuckets", () => {
    const wrapper = mount(PwaCacheActions, { props: { hasNoBuckets: true, isLoading: false } });
  ...
  it("Re-sync never disabled", () => {
    const wrapper = mount(PwaCacheActions, { props: { hasNoBuckets: true, isLoading: true } });
